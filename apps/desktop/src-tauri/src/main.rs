    ...
    let rows = statement
        .query_map([], |row| {
            ...
            Ok(SaleRecord {
                ...
                footer_note: row.get(11)?,
                issued_at: row.get(12)?,
                product_id: row.get(13)?,
                product_name: row.get(14)?,
                quantity: row.get(15)?,
                unit_price_minor: row.get(16)?,
                total_minor: row.get(17)?,
                lines: Vec::new(),
                payment_status: row.get(18)?,
                occurred_at_ms: row.get(19)?,
                occurred_at_label: row.get(20)?,
            })
        })
...
fn apply_migrations(connection: &Connection) -> Result<(), String> {
    ...
    ensure_column(
        ...
        "sales",
        "observations",
        "ALTER TABLE sales ADD COLUMN observations TEXT NOT NULL DEFAULT ''",
    )?;
    ensure_column(
        connection,
        "sales",
        "footer_note",
        "ALTER TABLE sales ADD COLUMN footer_note TEXT NOT NULL DEFAULT ''",
    )?;
    ensure_column(
        connection,
        "customer_receipts",
